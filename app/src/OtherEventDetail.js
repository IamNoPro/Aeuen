...
import './OtherEventDetail.css';
import mapboxgl from '!mapbox-gl'; // eslint-disable-line import/no-webpack-loader-syntax


function OtherEventDetail(props) {
    ...
    if (suggestedSong) {
        setTimeout(() => {setSuggestedSong(false)}, 5000);
    }

    if(requestedCollaboration) {
        setTimeout(() => {setRequestedCollaboration(false)}, 5000);
    }
    ...
    return (
        <div className={'content'}>
            {
                toggleSuggestModal && (
                    <div id='myModal' className={'modal'} onClick={() => setToggleSuggestModal(!toggleSuggestModal)}>
                        <div className={'modal-content'} onClick={(e) => e.stopPropagation()}>
                            <span className={'close'} onClick={() => setToggleSuggestModal(!toggleSuggestModal)}> &times;</span>
                            <h1> Suggest any song </h1>
                            <h4> Your suggestion: </h4>
                            <input className={'my-input'} type={'text'} placeholder={'Type in the name of your song/songs'}/>
                            <br/>
                            <div className={'modal-bottom'}>
                                <button className={'my-modal-button-submit'} onClick={() => {
                                    setToggleSuggestModal(!toggleSuggestModal);
                                    setSuggestedSong(true);
                                }} > SUGGEST </button>
                                <button className={'my-modal-button-cancel'} onClick={() => setToggleSuggestModal(!toggleSuggestModal)} > CANCEL </button>
                            </div>
                        </div>
                    </div>
                )
            ...
            {
                toggleRequestModal && (
                    <div id='myModal' className={'modal'} onClick={() => setToggleRequestModal(!toggleRequestModal)}>
                        <div className={'modal-content'} onClick={(e) => e.stopPropagation()}>
                            <span className={'close'} onClick={() => setToggleRequestModal(!toggleRequestModal)}> &times;</span>
                            <h1> {`Become a collaborator in ${eventInfo.host}'s ${eventInfo.title}!`} </h1>
                            <h4> Your message: </h4>
                            <input className={'my-input'} type={'text'} placeholder={'Type in something...'}/>
                            <br/>
                            <div className={'modal-bottom'}>
                                <button className={'my-modal-button-submit'} onClick={() => {
                                ...
                                <button className={'my-modal-button-cancel'} onClick={() => setToggleRequestModal(!toggleRequestModal)} > CANCEL </button>
                            </div>
                        </div>
                    </div>
                )
            ...
            <div className={'left-content'}>
                <div className={'vertical'}>
                    <button className={'my-button'} onClick={() => setToggleSuggestModal(!toggleSuggestModal)}> SUGGEST SONG </button>
                    <br/>
                    {
                        eventInfo.collaboration_status === 'none' ?
                            <button className={'my-button'} onClick={() => setToggleRequestModal(!toggleRequestModal)}> COLLABORATE </button>
                            :
                            <button className={'my-button-disabled'} disabled={true} > {eventInfo.collaboration_status.toUpperCase()} </button>
                    }
                </div>
            </div>
                ...
                <div className={'clickable-section'}>
                    ...
                    <div className={'organizers'}>
                        {eventInfo.organizers.map((organizer) => {
                            return <img className={'organizer-avatar'} src={organizer.avatar} height={65} width={65}/>
                        })}
                    </div>
                </div>
                <div className={'section'}>
...
export default OtherEventDetail;