...
const Signup = () => {
	...
	const addFirestore = uid => {
		...
		const uploadTask = storage
			.ref(`profile_images/${selectedFile.name}`)
	...
	const onSubmit = () => {
		...
		if (!selectedFile.name) {
			currentErrors.file = 'Profile photo is needed';
			validInputs = false;
		}
		if (!validInputs) {
			...
			<div className="mid-content container-create">
				<form className="create-event">
					...
					<div className="form-control form-control-login">
						...
						<label
							className="form-control-poster"
							style={{ width: selectedFile.file ? '275px' : '150px' }}
						>
